...
class Logger(object):
    ...
    def log(self, status, message, network_id=None, user=None):
        """
        写入日志
        :param message: 日志信息
        :param status: True表示运行信息，False表示错误信息
        :return:
        """
        if status:
            self.run_logger.info("%s - [%s] " % (user, network_id) + message)
        else:
            self.error_logger.error("%s - [%s] " % (user, network_id) + message)
